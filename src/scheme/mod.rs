...
pub(crate) fn get_env() -> Env {    SESSION_ENV.with(|e| {
...
pub fn dump_env() -> String {
    ...
    let mut lines = vec![
        "; ma-scheme session — generated by .my.scheme!save".to_string(),
        "; Edit with .my.scheme!edit  •  Reload with .my.scheme!eval".to_string(),
        String::new(),
    ];
...
fn val_to_define(name: &str, val: &SchemeVal) -> Option<String> {
    match val {
        ...
        SchemeVal::Lambda { params, rest, body, .. } => {
            let param_str = match rest {
                Some(r) if params.is_empty() => format!(". {r}"),
                Some(r) => format!("{} . {r}", params.join(" ")),
                None    => params.join(" "),
            };
            let body_fmt = body.iter().map(|e| format!("\n  {}", expr_to_src(e))).collect::<String>();
...
fn val_to_src(val: &SchemeVal) -> String {
    match val {
        ...
        SchemeVal::List(items) => format!("({})", items.iter().map(val_to_src).collect::<Vec<_>>().join(" ")),
        other                  => other.display(),
    }
}
...
fn expr_to_src(expr: &parser::SchemeExpr) -> String {
    match expr {
        ...
        parser::SchemeExpr::List(fs) => {
            format!("({})", fs.iter().map(expr_to_src).collect::<Vec<_>>().join(" "))
        }
    }
}
