...
label-nick = bijnaam
new-identity-nick-help = Kies een bijnaam voor 間-ruimte. Het is alleen een bijnaam en je kunt die op elk moment wijzigen.
...
qr-scan-hint = Richt de camera op een QR-code van een operator-profiel.
...
msg-logged-in = operator v{ $version } — aangemeld als { $username }
...
msg-auto-published = DID gepubliceerd via lokale 間 ({ $url })
...
msg-trusted-ma-lookup-wait = Het opzoeken van de vertrouwde MA kan tot { $seconds } seconden duren.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Identiteit wordt gepubliceerd. Dit kan tot { $seconds } seconden duren.
...
msg-identity-not-published = Identiteit niet online gevonden — als je 間 lokaal geïnstalleerd hebt, voer '.ma: claim [port]' uit en dan '.my.identity!publish @ma'. Typ '.help/publish' voor details.
...
discover-hint-endpoint-not-found = Hint: eindpunt niet gevonden. Controleer dat 間 draait en /status.json op poort 5003 aanbiedt.
discover-hint-server-error = Hint: runtime antwoordde met een serverfout. Controleer de 間-logs en probeer opnieuw.
discover-hint-network = Hint: netwerk-/verbindingsprobleem. Start 間, controleer dat localhost:5003 bereikbaar is en sta lokale HTTP-toegang in de browser toe.
discover-hint-generic = Hint: controleer dat 間 en IPFS Desktop draaien en probeer `.ma` opnieuw.
discover-success = 間 gedetecteerd bij { $url }
...
doc-publish-hint-session = meld je opnieuw aan zodat operator toegang heeft tot je identiteitssleutels
doc-publish-hint-target = gebruik een geldige publisher-DID of alias die naar een kale did:ma:<ipns> verwijst
doc-publish-hint-network = controleer dat 間-runtime en IPFS bereikbaar zijn en probeer opnieuw
...
# ── Helptekst — koppen ────────────────────────────────────────────────────
help-header-operator = ── operator-opdrachten ────────────────────────────────────────────────────────
...
help-runtime-discover =   .ma: claim [port]          lokale runtime detecteren en @ma-alias aanmaken
help-runtime-claim =   .ma: claim [port]             runtime claimen met uw DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   verbind met je lokale 間-runtime, lees /status.json en sla .ma.ctx.* op
help-ma-publish =   .my.identity!publish @ma     publiceer je DID-document zodat anderen je sleutels en endpoint kunnen vinden
help-ma-security = De duidelijkste vertrouwensgrens is je eigen 間-runtime met je eigen IPFS Desktop/Kubo. Een externe publisher kan nuttig zijn, maar dan vertrouw je op de dienst van iemand anders.
...
help-topic-url =   .help/url                    operator openen via een URL-link
...
help-url-intro =   Deel een link die operator opent met een vooraf ingevulde ontvanger:
...
help-publish-ma = Om te publiceren heb je 間 (lokale runtime) nodig. Het koppelt operator aan IPFS/IPNS namens jou.
help-publish-steps = Stappen: voer '.ma: claim [port]' uit om je lokale 間 te detecteren, dan '.my.identity!publish @ma'.
...
help-zscheme-intro = zscheme evalueert Scheme-expressies die in operator-opdrachten zijn ingebed en voegt het resultaat vóór verzending in de regel in.
...
label-local-ipfs = Lokale IPFS-gateway (vereist browseruitzondering)

tab-config = Instellingen
