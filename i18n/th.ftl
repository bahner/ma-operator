...
msg-identity-not-published = ไม่พบตัวตนออนไลน์ — ถ้าติดตั้ง 間 ในเครื่องแล้ว ให้รัน '.ma: claim [port]' จากนั้น '.my.identity!publish @ma' พิมพ์ '.help/publish' เพื่อดูรายละเอียด
...
discover-hint-endpoint-not-found = คำแนะนำ: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = คำแนะนำ: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = คำแนะนำ: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = คำแนะนำ: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = พบ 間 ที่ { $url }
...
help-runtime-discover =   .ma: claim [port]          ค้นหา runtime ท้องถิ่นและสร้าง @ma
help-runtime-claim =   .ma: claim [port]             อ้างสิทธิ์ runtime ด้วย DID ของคุณ
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   เชื่อมต่อกับ 間 runtime ในเครื่อง อ่าน /status.json แล้วบันทึก .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     เผยแพร่เอกสาร DID ของคุณ เพื่อให้ผู้อื่นค้นหากุญแจและ endpoint ของคุณได้
help-ma-security = ขอบเขตความไว้วางใจที่ชัดที่สุดคือ 間 runtime ของคุณเองพร้อม IPFS Desktop/Kubo ของคุณเอง publisher ระยะไกลอาจมีประโยชน์ แต่คุณจะพึ่งพาบริการของคนอื่น
