...
msg-identity-not-published = Tożsamość nie znaleziona online — jeśli masz 間 zainstalowane lokalnie, uruchom '.ma [port]', a potem '.my.identity!publish @ma'. Wpisz '.help/publish' po szczegóły.
...
help-ma-command =   .ma [port]                   połącz się z lokalnym 間 runtime, odczytaj /status.json i zapisz .ma.ctx.*
