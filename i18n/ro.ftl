...
msg-identity-not-published = Identitate negăsită online — dacă ai 間 instalat local, rulează '.ma [port]' apoi '.my.identity!publish @ma'. Tastează '.help/publish' pentru detalii.
...
help-ma-command =   .ma [port]                   conectează-te la runtime-ul 間 local, citește /status.json și salvează .ma.ctx.*
