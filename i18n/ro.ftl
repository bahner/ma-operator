...
msg-auto-published = DID publicat prin 間 local ({ $url })
...
msg-trusted-ma-lookup-wait = Căutarea MA de încredere poate dura până la { $seconds } secunde.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Se publică identitatea. Acest lucru poate dura până la { $seconds } secunde.
...
msg-identity-not-published = Identitate negăsită online — dacă ai 間 instalat local, rulează '.ma: claim [port]' apoi '.my.identity!publish @ma'. Tastează '.help/publish' pentru detalii.
...
discover-hint-endpoint-not-found = Indiciu: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Indiciu: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Indiciu: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Indiciu: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 descoperit la { $url }
...
err-persist = eroare la salvare: { $e }
...
help-runtime-discover =   .ma: claim [port]          descoperă runtime local și creează alias @ma
help-runtime-claim =   .ma: claim [port]             revendică runtime cu DID-ul tău
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   conectează-te la runtime-ul 間 local, citește /status.json și salvează .ma.ctx.*
...
help-ma-security = Cea mai clară limită de încredere este propriul tău runtime 間 cu propriul IPFS Desktop/Kubo. Un publisher la distanță poate fi util, dar atunci te bazezi pe serviciul altcuiva.
...
label-local-ipfs = Gateway IPFS local (necesită excepție de browser)
