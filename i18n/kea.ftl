...
msg-auto-published = DID pablikadu via 間 lokal ({ $url })
...
msg-trusted-ma-lookup-wait = Buska di MA di konfiansa podi leba ti { $seconds } segundus.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Ta publika identidad. Es podi leba ti { $seconds } segundus.
...
msg-identity-not-published = Idantidadi ka acha na intanet — si bu ten 間 instaladu lokal, kori '.ma: claim [port]' dipus '.my.identity!publish @ma'. Skriva '.help/publish' pa ditailyu.
...
discover-hint-endpoint-not-found = Dika: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Dika: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Dika: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Dika: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 deskubertu na { $url }
...
err-persist = érro di persistênsia: { $e }
...
doc-store-sent = pedidu di armazenamentu mandadu ({ $id }) → { $publisher }; CID ta xiga via risposta
doc-ipld-store-sent = pedidu di armazenamentu IPLD mandadu ({ $id }) → { $publisher }; CID ta xiga via risposta
...
help-cmd-history =   .history [n]                 istória di komandu (kopia konsekutivu meskladu); n opsional ta mostra sô últimu n liña
...
help-msg-send = @alias!msg body @alias:verb args manda mensajen pa atór
...
help-config-filter =   .path [nomi]                 lista nomis di xavis fidju ki ten nomi
help-config-full =   .path. [nomi]                lista valoris di sób-árvori interu ki ten nomi
...
help-runtime-discover =   .ma: claim [port]          diskubrí runtime lokal i kria alias @ma
help-runtime-claim =   .ma: claim [port]             reklama propriedadi di runtime ku bo DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   liga ku bu 間 runtime lokal, le /status.json, y guarda .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     publika bu dokumentu DID pa otus pudi risolvi bu xabis y endpoint
help-ma-security = Limiti di konfiansa mas klaru é bu propi 間 runtime ku bu propi IPFS Desktop/Kubo. Un publisher lonji pudi ser útil, mas enton bu ta dipendi di sirvisu di otu algin.
...
err-history-count-invalid = kantidadi di istória tem ki ser un númeru interu pozitivu
