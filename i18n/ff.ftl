...
msg-identity-not-published = Ko tigi alaa woyowoyii dow — so 間 sappidi dow, laato '.ma: claim [port]' ɓe '.my.identity!publish @ma'. Windu '.help/publish' ngam tabitinde.
...
discover-hint-endpoint-not-found = Tinndinoore: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Tinndinoore: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Tinndinoore: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Tinndinoore: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 yiytaama e { $url }
...
doc-store-sent = gollal mooftude nawnaaɗo ({ $id }) → { $publisher }; CID ara e jaabiri
doc-ipld-store-sent = gollal mooftude IPLD nawnaaɗo ({ $id }) → { $publisher }; CID ara e jaabiri
...
help-msg-send = @alias!msg body @alias:verb args nawtu koɗe e actor
...
help-runtime-discover =   .ma: claim [port]          yiytude runtime tiiɗnde e hobbaade hesbu @ma
help-runtime-claim =   .ma: claim [port]             ɗaɓɓude runtime DID maa
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   seŋo e local 間 runtime maa, jang /status.json, tee danndu .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     jaɓɓin DID document maa ngam woɗɓe mbaawa yiytude key maa e endpoint maa
help-ma-security = Ɗo ɓuri laaɓde e koolnol ko 間 runtime maa tigi e IPFS Desktop/Kubo maa tigi. Remote publisher ena waawi wallude, kono ɗuum noon ko huutorto-ɗaa sarwiis goɗɗo.
...
# ── redigering ─────────────────────────────────────────────────────
term-error = juumre
term-error-detail = juumre: { $detail }
...
help-topic-ma = .help/ma                     間-nokku, baylugol, e naatgol
