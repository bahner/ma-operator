...
msg-identity-not-published = 未在线找到身份 — 如果本地安装了 間，请运行 '.ma: claim [port]' 然后 '.my.identity!publish @ma'。输入 '.help/publish' 查看详情。
...
discover-hint-endpoint-not-found = 提示：未找到端点。请确认 間 正在运行，并在 5003 端口公开 /status.json。
discover-hint-server-error = 提示：运行时返回了服务器错误。请检查 間 日志并重试。
discover-hint-network = 提示：网络/连接问题。启动 間，确认 localhost:5003 可访问，并在浏览器中允许本地 HTTP 访问。
discover-hint-generic = 提示：确认 間 和 IPFS Desktop 正在运行，然后重试 `.ma`。
discover-success = 已在 { $url } 发现 間
...
# ── 动词 — 文档 ───────────────────────────────────────────────────────────
doc-content-empty = { $path } 为空
doc-save-first = { $path } 为空 — 请先保存
...
doc-store-sent = 存储请求已发送（{ $id }）→ { $publisher }；CID 将通过 回复到达
doc-ipld-store-sent = IPLD 存储请求已发送（{ $id }）→ { $publisher }；CID 将通过 回复到达
doc-fetch-done = 已获取 { $cid } → { $path }（未执行）
...
help-cmd-history =   .history [n]                 命令历史（连续重复项已合并）；可选的 n 只显示最后 n 行
help-cmd-logout =   .logout                      退出登录
help-cmd-batch =   .batch                       并行执行临时文档
help-cmd-batch-sync =   .batch!sync [timeout=60s] / .batch         逐行执行临时文档

# ── 帮助文本 — 消息传递 ───────────────────────────────────────────────────
help-msg-echo =   @alias                       显示已解析的 DID/DID-URL（不发送消息）
help-msg-send = @alias!msg body @alias:verb args 发送消息 给角色
...
help-config-filter =   .path [名称]                 列出包含名称的子键名称
help-config-full =   .path. [名称]                列出包含名称的完整子树值
...
help-runtime-discover =   .ma: claim [port]          发现本地运行时并创建 @ma 别名
help-runtime-claim =   .ma: claim [port]             用您的 DID 声明运行时所有权
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   连接本地 間 runtime，读取 /status.json，并保存 .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     发布你的 DID 文档，让其他人能解析你的密钥和 endpoint
help-ma-security = 最清晰的信任边界，是你自己的 間 runtime 加上你自己的 IPFS Desktop/Kubo。远程 publisher 可能有用，但那意味着你依赖别人的服务。
...
err-history-count-invalid = 历史记录数量必须为正整数
...
help-topic-ma = .help/ma                     間-空间、发布、入口
