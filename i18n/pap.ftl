...
msg-auto-published = DID publiká via 間 lokal ({ $url })
...
msg-trusted-ma-lookup-wait = Búskeda di MA konfiá por tuma te { $seconds } sekònde.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Ta publiká identidat. Esaki por tuma te { $seconds } sekònde.
...
msg-identity-not-published = Identidad no a haña na internet — si 間 ta instalá lokalmente, drenta '.ma: claim [port]' i despues '.my.identity!publish @ma'. Tik '.help/publish' pa detaljenan.
...
discover-hint-endpoint-not-found = Pista: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Pista: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Pista: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Pista: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 diskubrí na { $url }
...
# ── Verbs — documents ────────────────────────────────────────────────────
doc-content-empty = { $path } ta bashi
doc-save-first = { $path } ta bashi — wardá promé
...
doc-store-sent = petishon di almacenamentu mandá ({ $id }) → { $publisher }; CID lo yega via respuesta
doc-ipld-store-sent = petishon di almacenamentu IPLD mandá ({ $id }) → { $publisher }; CID lo yega via respuesta
doc-fetch-done = bishitá { $cid } → { $path } (no ehekulá)
...
help-msg-send = @alias!msg body @alias:verb args mandá mensahe na aktor
...
help-config-filter =   .path [nòmber]              lista nòmbernan di yabi ku ta kontené nòmber
help-config-full =   .path. [nòmber]             lista e balornan kompleto di subarber ku ta kontené nòmber
...
help-runtime-discover =   .ma: claim [port]          deskubrí runtime lokal i kreá alias @ma
help-runtime-claim =   .ma: claim [port]             reklamá poseeshon di runtime ku bo DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   konektá ku bo 間 runtime lokal, lesa /status.json, i warda .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     publiká bo dokumento DID pa otro hende por resolve bo yabi nan i endpoint
help-ma-security = E frontera di konfiansa mas kla ta bo propio 間 runtime ku bo propio IPFS Desktop/Kubo. Un publisher remoto por ta útil, pero e ora ei bo ta dependé di servisio di otro hende.
...
help-topic-ma = .help/ma                     間-espasio, publikashon, i entrada
