...
lang-name = Español
label-nick = apodo
new-identity-nick-help = Elige un apodo para 間-space. Es solo un apodo y puedes cambiarlo cuando quieras.
...
btn-scan-qr = escanear QR
qr-scan-hint = Apunta la cámara a un código QR de perfil de operator.
qr-error-camera = cámara no disponible: { $e }
qr-error-too-large = perfil demasiado grande para un código QR
...
msg-logged-in = operator v{ $version } — sesión iniciada como { $username }
...
msg-auto-published = DID publicado a través de 間 local ({ $url })
...
msg-trusted-ma-lookup-wait = La búsqueda del MA de confianza puede tardar hasta { $seconds } segundos.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Publicando la identidad. Esto puede tardar hasta { $seconds } segundos.
...
msg-identity-not-published = Identidad no encontrada en línea — si tienes 間 instalado localmente, ejecuta '.ma: claim [port]' y luego '.my.identity!publish @ma'. Escribe '.help/publish' para más detalles.
...
discover-hint-endpoint-not-found = Pista: punto final no encontrado. Comprueba que 間 esté ejecutándose y exponga /status.json en el puerto 5003.
discover-hint-server-error = Pista: el runtime respondió con un error del servidor. Revisa los registros de 間 e inténtalo de nuevo.
discover-hint-network = Pista: problema de red/conectividad. Inicia 間, verifica que localhost:5003 sea accesible y permite acceso HTTP local en el navegador.
discover-hint-generic = Pista: verifica que 間 e IPFS Desktop estén en ejecución y reintenta `.ma`.
discover-success = 間 descubierto en { $url }
...
doc-publish-hint-session = inicia sesión otra vez para que operator pueda acceder a tus claves de identidad
doc-publish-hint-target = usa una DID de publicador válida o un alias que resuelva a una did:ma:<ipns> sin fragmento
doc-publish-hint-network = verifica que el runtime 間 e IPFS sean accesibles y reintenta
...
doc-store-sent = solicitud de almacenamiento enviada ({ $id }) → { $publisher }; CID llegará por respuesta
doc-ipld-store-sent = solicitud de almacenamiento IPLD enviada ({ $id }) → { $publisher }; CID llegará por respuesta
...
help-header-operator = ── comandos operator ──────────────────────────────────────────────────────────
...
help-cmd-history =   .history [n]                 historial de comandos (duplicados consecutivos combinados); n opcional muestra solo las últimas n líneas
...
help-msg-send = @alias!msg body @alias:verb args enviar mensaje a actor
...
help-config-filter =   .path [nombre]               listar nombres de claves hijas que contienen nombre
help-config-full =   .path. [nombre]              listar valores completos del subárbol que contienen nombre
...
help-runtime-discover =   .ma: claim [port]          descubrir tiempo de ejecución local y crear alias @ma
help-runtime-claim =   .ma: claim [port]             reclamar tiempo de ejecución con tu DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   conectar con tu runtime 間 local, leer /status.json y guardar .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     publicar tu documento DID para que otros resuelvan tus claves y tu endpoint
help-ma-security = El límite de confianza más claro es tu propio runtime 間 con tu propio IPFS Desktop/Kubo. Un publisher remoto puede ser útil, pero entonces dependes del servicio de otra persona.
...
err-history-count-invalid = el número de historial debe ser un entero positivo
...
help-topic-ma = .help/ma                     間-espacio, publicación y entrada
...
help-topic-z =   .help/z                      contenido Scheme público y privado
help-topic-zscheme =   .help/zscheme               expresiones Scheme en línea y documentación
help-topic-url =   .help/url                    abrir operator mediante un enlace URL
...
help-url-intro =   Comparte un enlace que abre operator con un destinatario ya rellenado:
...
help-publish-ma = Para publicar necesitas 間 (runtime local) instalado. Conecta operator con IPFS/IPNS en tu nombre.
help-publish-steps = Pasos: ejecuta '.ma: claim [port]' para detectar tu 間 local, luego '.my.identity!publish @ma'.
...
msg-outbox-open-timeout = el actor { $target } no está disponible — la conexión agotó el tiempo tras { $ms }ms
msg-outbox-send-timeout = el envío a { $target } agotó el tiempo tras { $ms }ms — puede que la conexión se haya cortado
...
help-z-public =   .z.*                       contenido público o publicable; no guardes secretos aquí
help-z-scheme =   .z.scheme                  el script Scheme que se carga y evalúa al iniciar
help-z-control =   .z.scheme                 tú controlas el contenido del script de inicio
help-z-private =   .my.*                     configuración local privada; también puedes guardar Scheme aquí
help-z-choice =   .z.* / .my.*               usa .z.* para Scheme público y .my.* para Scheme privado
help-header-zscheme-topic = ── zscheme ──────────────────────────────────────────────────────────────
help-zscheme-intro = zscheme evalúa expresiones Scheme incrustadas en comandos de operator e inserta el resultado en la línea antes de enviarla.
