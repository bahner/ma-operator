...
lang-name = Hrvatski
label-nick = nadimak
new-identity-nick-help = Odaberite nadimak za 間-space. To je samo nadimak i možete ga promijeniti bilo kada.
...
qr-scan-hint = Usmjerite kameru prema QR kodu operator profila.
qr-error-camera = kamera nije dostupna: { $e }
qr-error-too-large = profil je prevelik za QR kod
...
msg-logged-in = operator v{ $version } — prijavljeni kao { $username }
msg-type-help = Unesite .help za popis naredbi.
msg-getting-started = Pokušajte: say Hello world, look, .help ili help.
msg-connecting = spajanje na iroh...
msg-iroh-ready = iroh krajnja točka spremna
msg-iroh-failed = iroh: { $e }
msg-auto-published = DID objavljen putem lokalnog 間 ({ $url })
...
msg-trusted-ma-lookup-wait = Pretraga pouzdanog MA-a može potrajati do { $seconds } sekundi.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Objavljuje se identitet. To može potrajati do { $seconds } sekundi.
...
msg-identity-not-published = Identitet nije pronađen online — ako imaš 間 instaliran lokalno, pokreni '.ma: claim [port]' i zatim '.my.identity!publish @ma'. Upiši '.help/publish' za pojedinosti.
...
discover-hint-endpoint-not-found = Savjet: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Savjet: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Savjet: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Savjet: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 otkriveno na { $url }
...
err-persist = greška pri spremanju: { $e }
...
doc-save-first = { $path } je prazno — prvo spremite
...
doc-publish-hint-session = ponovno se prijavite kako bi operator mogao pristupiti vašim identifikacijskim ključevima
...
help-header-operator = ── naredbe operator ───────────────────────────────────────────────────────────
...
help-runtime-discover =   .ma: claim [port]          otkrij lokalno izvođenje i stvori pseudonim @ma
help-runtime-claim =   .ma: claim [port]             zahtijevaj izvođenje svojim DID-om
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   poveži se s lokalnim ma runtimeom, pročitaj /status.json i spremi .ma.ctx.*
...
help-ma-security = Najjasnija granica povjerenja je tvoj vlastiti 間 runtime s vlastitim IPFS Desktop/Kubo. Udaljeni publisher može biti koristan, ali tada se oslanjaš na tuđu uslugu.
...
help-topic-url =   .help/url                    otvaranje operatora putem URL veze
help-topic-i18n = .help/i18n                   preferencija jezika za vaš identitet
help-header-url = ── URL parametri ────────────────────────────────────────────────────────────────
help-url-intro =   Podijeli vezu koja otvara operator s unaprijed ispunjenim primateljem:
...
help-publish-ma = Za objavu trebaš instaliran 間 (lokalno izvršno okruženje). Ono povezuje operator s IPFS/IPNS u tvoje ime.
help-publish-steps = Koraci: pokreni '.ma: claim [port]' za otkrivanje lokalnog ma, zatim '.my.identity!publish @ma'.
...
help-zscheme-intro = zscheme vrednuje Scheme izraze ugrađene u operator naredbe i umeće rezultat u redak prije slanja.
help-zscheme-inline =   > say 7 + 5 = (+ 7 5)    umetnuti primjer; šalje "say 7 + 5 = 12"
help-zscheme-define =   (define x 12)             definicije ostaju tijekom trenutne prijavne sesije
help-zscheme-doc = Dokumentacija: https://github.com/bahner/rust-ma-zscheme

label-local-ipfs = Lokalni IPFS pristupnik (potreban izuzetak preglednika)
