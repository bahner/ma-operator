...
msg-identity-not-published = Tapatybė nerasta internete — jei 間 įdiegtas vietoje, paleisk '.ma: claim [port]' ir tada '.my.identity!publish @ma'. Parašyk '.help/publish' dėl detalių.
...
discover-hint-endpoint-not-found = Užuomina: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Užuomina: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Užuomina: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Užuomina: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 aptikta { $url }
...
err-persist = išsaugojimo klaida: { $e }
...
doc-store-sent = saugojimo užklausa išsiųsta ({ $id }) → { $publisher }; CID ateis per atsakymą
doc-ipld-store-sent = IPLD saugojimo užklausa išsiųsta ({ $id }) → { $publisher }; CID ateis per atsakymą
...
help-msg-send = @alias!msg body @alias:verb args siųsti pranešimą dalyviui
...
help-runtime-discover =   .ma: claim [port]          aptikti vietinį vykdymo laiką ir sukurti @ma pseudonimą
help-runtime-claim =   .ma: claim [port]             pareikšti vykdymo laiką savo DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   prisijunk prie vietinio 間 runtime, perskaityk /status.json ir išsaugok .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     paskelbk savo DID dokumentą, kad kiti galėtų rasti tavo raktus ir endpoint
help-ma-security = Aiškiausia pasitikėjimo riba yra tavo paties 間 runtime su tavo paties IPFS Desktop/Kubo. Nuotolinis publisher gali būti naudingas, bet tada remiesi kito žmogaus paslauga.
...
# ── redigering ─────────────────────────────────────────────────────
term-error = klaida
term-error-detail = klaida: { $detail }
...
help-topic-ma = .help/ma                     間-erdvė, leidyba ir įėjimas
