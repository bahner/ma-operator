...
msg-trusted-ma-lookup-wait = A search fi trusted MA can tek up to { $seconds } seconds.
...
doc-store-sent = store request forward ({ $id }) → { $publisher }; CID will arrive via
doc-ipld-store-sent = IPLD store request forward ({ $id }) → { $publisher }; CID will arrive via
...
help-msg-send = @alias!msg body @alias:verb args send message to I-dren
...
help-runtime-discover =   .ma: claim [port]          discover local runtime and manifest @ma I-name
help-runtime-claim =   .ma: claim [port]             claim runtime ownership with I and I DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   link to yuh local 間 runtime, read /status.json, an save .ma.ctx.*
...
# ── redigering ─────────────────────────────────────────────────────
term-error = iration failin
term-error-detail = iration failin: { $detail }
