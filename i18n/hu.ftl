...
msg-identity-not-published = Az identitás nem található online — ha helyileg telepítve van a 間, futtasd '.ma: claim [port]', majd '.my.identity!publish @ma'. Írd '.help/publish' a részletekért.
...
discover-hint-endpoint-not-found = Tipp: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Tipp: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Tipp: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Tipp: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 felderítve a következő helyen: { $url }
...
help-runtime-discover =   .ma: claim [port]          helyi futtatókörnyezet felderítése és @ma álnév létrehozása
help-runtime-claim =   .ma: claim [port]             futtatókörnyezet igénybevétele a DID-del
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   kapcsolódj a helyi 間 runtime-hoz, olvasd be a /status.json-t, és mentsd a .ma.ctx.* értékeket
...
help-ma-security = A legtisztább bizalmi határ a saját 間 runtime-od a saját IPFS Desktop/Kubo példányoddal. Egy távoli publisher hasznos lehet, de akkor valaki más szolgáltatására támaszkodsz.
