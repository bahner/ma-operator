...
msg-auto-published = DID don publish through local 間 ({ $url })
...
msg-trusted-ma-lookup-wait = Trusted MA lookup fit take up to { $seconds } seconds.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Dey publish identity. Dis fit take up to { $seconds } seconds.
...
msg-identity-not-published = Identity no dey online — if 間 don install for your side, run '.ma: claim [port]' den '.my.identity!publish @ma'. Type '.help/publish' for details.
...
discover-hint-endpoint-not-found = Hint: endpoint no dey. Check say 間 dey show /status.json for port 5003.
discover-hint-server-error = Hint: runtime return server error. Check 間 logs and try again.
...
discover-success = 間 discovered for { $url }
...
doc-store-sent = store request send ({ $id }) → { $publisher }; CID go arrive via reply
doc-ipld-store-sent = IPLD store request send ({ $id }) → { $publisher }; CID go arrive via reply
...
help-msg-send = @alias!msg body @alias:verb args send mesej go aktor
...
help-config-filter =   .path [name]                 list pikin key name we get name
help-config-full =   .path. [name]                list full sabtri valu we get name
...
help-runtime-discover =   .ma: claim [port]          diskova lokal runtime and kreate @ma alias
help-runtime-claim =   .ma: claim [port]             klaim ownashop of runtime wit yu DID
...
help-ma-command = .ma: claim [port] / .ma: did:ma:… / .ma                   connect to your local 間 runtime, read /status.json, and save .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     publish your DID document make odas fit resolve your keys and endpoint
help-ma-security = Di clearest trust boundary na your own 間 runtime with your own IPFS Desktop/Kubo. Remote publisher fit help, but den you dey depend on anoda person service.
...
help-ma-entry-steps = Start IPFS Desktop and 間, then run .ma. Publish with .my.identity!publish @ma, find world, and enter with .enter @ma.
