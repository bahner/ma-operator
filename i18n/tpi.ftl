...
msg-auto-published = DID i bin publishim long local 間 ({ $url })
...
msg-trusted-ma-lookup-wait = Painim trusted MA inap kisim inap { $seconds } seken.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Wok long publishim identity. Dispela inap kisim inap { $seconds } seken.
...
msg-identity-not-published = Nem bilong yu i no stap long net — sapos 間 i istap long komputa bilong yu, ran '.ma: claim [port]' na '.my.identity!publish @ma'. Rait '.help/publish' bilong kisim sampela save.
...
discover-hint-endpoint-not-found = Toksave: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Toksave: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Toksave: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Toksave: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 discoveri long { $url }
...
help-config-filter =   .path [nem]                  soim ol pikinini ki nem bilong ol i gat nem
help-config-full =   .path. [nem]                 soim olgeta valu bilong sabpatri i gat nem
...
help-runtime-discover =   .ma: claim [port]          painim lokal runtime na mekim alias @ma
help-runtime-claim =   .ma: claim [port]             klemim ownashop bilong runtime long yu DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   connect long local 間 runtime bilong yu, ritim /status.json, na save .ma.ctx.*
...
help-ma-security = Klir trust boundary em own 間 runtime bilong yu wantaim own IPFS Desktop/Kubo bilong yu. Remote publisher inap help, tasol bihain yu depend long service bilong narapela.
