...
msg-trusted-ma-lookup-wait = 信頼できる MA の検索は最大 { $seconds } 秒かかる場合があります。
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = identity を公開しています。最大 { $seconds } 秒かかる場合があります。
...
msg-identity-not-published = オンラインで身元が見つかりません — maがローカルにインストールされている場合、'.ma: claim [port]'を実行してから'.my.identity!publish @ma'を実行してください。詳細は'.help/publish'と入力してください。
...
help-runtime-discover =   .ma: claim [port]          ローカルランタイムを検出し @ma エイリアスを作成
help-runtime-claim =   .ma: claim [port]             自分の DID でランタイムを要求
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   ローカルの 間 runtime に接続し、/status.json を読み、.ma.ctx.* を保存する
...
help-publish-steps = 手順：'.ma: claim [port]'でローカルmaを検出し、'.my.identity!publish @ma'を実行してください。
