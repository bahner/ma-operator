...
msg-identity-not-published = Atụfuọla njirimara n'ịntanetị — ọ bụrụ na 間 edepụtara n'ebe obibi, bido '.ma: claim [port]' wee mee '.my.identity!publish @ma'. Dee '.help/publish' maka nkọwa.
...
discover-hint-endpoint-not-found = Ndụmọdụ: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Ndụmọdụ: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Ndụmọdụ: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Ndụmọdụ: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = achọpụtara 間 na { $url }
...
help-runtime-discover =   .ma: claim [port]          chọpụta runtime obibi ma mepụtakwa nchọkwa @ma
help-runtime-claim =   .ma: claim [port]             kwụa akwụ runtime site na DID gị
...
# ── Help text — ma-space ──────────────────────────────────────────────────
help-ma-intro = Ụlọ 間 bụ oghere dị n’etiti njirimara 間. 間 na-eme ka njirimara ndị a chọta ibe ha ma kwurịta okwu; mgbe e bipụtara njirimara gị, ị nwere ike isonye.
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   jikọọ na 間 runtime nke mpaghara, gụọ /status.json, ma chekwaa .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     bipụta akwụkwọ DID gị ka ndị ọzọ nwee ike ịchọta igodo gị na endpoint gị
help-ma-security = Oke ntụkwasị obi kacha doo anya bụ 間 runtime nke gị na IPFS Desktop/Kubo nke gị. Publisher dị anya nwere ike ịba uru, mana mgbe ahụ ị na-adabere na ọrụ onye ọzọ.
