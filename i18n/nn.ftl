...
msg-identity-not-published = Identitet ikkje funne online — om du har 間 installert lokalt, køyr '.ma [port]' og deretter '.my.identity!publish @ma'. Skriv '.help/publish' for detaljar.
...
help-ma-command =   .ma [port]                   kopla til lokal 間-runtime, les /status.json, og lagra .ma.ctx.*
...
profile-no-ma = ingen 間-køyretid konfigurert — køyr '.ma [port]' fyrst
...
help-publish-steps = Steg: køyr '.ma [port]' for å oppdage din lokale 間, deretter '.my.identity!publish @ma'.
