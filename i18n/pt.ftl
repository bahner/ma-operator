...
discover-hint-endpoint-not-found = Dica: endpoint não encontrado. Verifique se `ma` está em execução e expõe /status.json na porta 5003.
discover-hint-server-error = Dica: o runtime respondeu com erro de servidor. Verifique os logs de `ma` e tente novamente.
discover-hint-network = Dica: problema de rede/conectividade. Inicie `ma`, verifique se localhost:5003 está acessível e permita acesso HTTP local no navegador.
discover-hint-generic = Dica: verifique se `ma` e IPFS Desktop estão em execução e tente `.ma` novamente.
...
doc-publish-hint-network = verifique se o runtime ma e o IPFS estão acessíveis e tente novamente
...
help-runtime-discover =   .ma [port]          descobrir tempo de execução local e criar alias @ma
help-runtime-claim =   .ma [port]             reclamar tempo de execução com o seu DID
...
help-ma-command =   .ma [port]                   ligar ao teu runtime ma local, ler /status.json e guardar .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     publicar o teu documento DID para que outros resolvam as tuas chaves e endpoint
help-ma-security = A fronteira de confiança mais clara é o teu próprio runtime ma com o teu próprio IPFS Desktop/Kubo. Um publisher remoto pode ser útil, mas então dependes do serviço de outra pessoa.
...
help-ma-entry-leave =   .leave                       sair da sala; a tua identidade continua ativa e continuas autenticado
