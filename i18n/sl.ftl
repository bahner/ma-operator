# operator — Slovenščina
lang-name = Slovenščina
label-nick = vzdevek
new-identity-nick-help = Izberite vzdevek za 間-space. To je samo vzdevek in ga lahko kadar koli spremenite.
...
qr-scan-hint = Usmerite kamero v QR kodo profila operator.
...
msg-logged-in = operator v{ $version } — prijavljeni kot { $username }
...
msg-identity-not-published = Identiteta ni najdena v omrežju — če imaš 間 nameščen lokalno, poženi '.ma: claim [port]', nato '.my.identity!publish @ma'. Vtipkaj '.help/publish' za podrobnosti.
...
discover-hint-endpoint-not-found = Namig: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Namig: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Namig: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Namig: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 odkrito na { $url }
...
doc-publish-hint-session = znova se prijavite, da bo lahko operator dostopal do vaših identifikacijskih ključev
...
doc-store-sent = zahteva za shranjevanje poslana ({ $id }) → { $publisher }; CID bo prišel v odgovoru
doc-ipld-store-sent = zahteva za shranjevanje IPLD poslana ({ $id }) → { $publisher }; CID bo prišel v odgovoru
...
help-header-operator = ── ukazi operator ────────────────────────────────────────────────────────────
...
help-cmd-history =   .history [n]                 zgodovina ukazov (zaporedni dvojniki združeni); neobvezni n prikaže samo zadnjih n vrstic
...
help-msg-send = @alias!msg body @alias:verb args pošlji sporočilo akterju
...
help-config-filter =   .path [ime]                  izpiši imena podključev, ki vsebujejo ime
help-config-full =   .path. [ime]                 izpiši vse vrednosti poddrevesa, ki vsebujejo ime
...
help-runtime-discover =   .ma: claim [port]          odkrij lokalno izvajanje in ustvari vzdevek @ma
help-runtime-claim =   .ma: claim [port]             zahtevaj izvajanje s svojim DID-om
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   poveži se z lokalnim 間 runtime, preberi /status.json in shrani .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     objavi svoj dokument DID, da lahko drugi razrešijo tvoje ključe in endpoint
help-ma-security = Najjasnejša meja zaupanja je tvoj lastni 間 runtime z lastnim IPFS Desktop/Kubo. Oddaljeni publisher je lahko koristen, vendar se takrat zanašaš na storitev nekoga drugega.
...
yaml-serialise-error = Napaka pri serializaciji YAML: { $e }
...
err-history-count-invalid = število zgodovine mora biti pozitivno celo število
...
# ── redigering ─────────────────────────────────────────────────────
term-error = napaka
term-error-detail = napaka: { $detail }
...
help-topic-ma = .help/ma                     間-prostor, založništvo in vstop
...
help-topic-z =   .help/z                      javna in zasebna vsebina Scheme
help-topic-zscheme =   .help/zscheme               vstavljeni izrazi Scheme in dokumentacija
...
help-topic-url =   .help/url                    odpiranje operator prek URL povezave
help-topic-i18n = .help/i18n                   nastavitev jezika za vašo identiteto
help-header-url = ── URL parametri ────────────────────────────────────────────────────────────────
help-url-intro =   Deli povezavo, ki odpre operator z vnaprej izpolnjenim prejemnikom:
...
help-publish-ma = Za objavo potrebuješ nameščen 間 (lokalno izvajalno okolje). Poveže operator z IPFS/IPNS v tvojem imenu.
help-publish-steps = Koraki: poženi '.ma: claim [port]' za zaznavo lokalnega ma, nato '.my.identity!publish @ma'.
...
msg-outbox-open-timeout = akter { $target } ni dosegljiv — povezava je potekla po { $ms }ms
msg-outbox-send-timeout = pošiljanje na { $target } je poteklo po { $ms }ms — povezava je morda prekinjena
...
# -- Help text -- z and my
help-header-z = ── .z in .my ───────────────────────────────────────────────────────────────
help-z-public =   .z.*                       javna ali objavljena vsebina; tukaj ne shranjujte skrivnosti
help-z-scheme =   .z.scheme                  skript Scheme, ki se naloži in ovrednosti ob zagonu
help-z-control =   .z.scheme                 vsebino zagonskega skripta nadzorujete sami
help-z-private =   .my.*                     zasebna lokalna konfiguracija; Scheme se lahko shrani tudi tukaj
help-z-choice =   .z.* / .my.*               uporabite .z.* za javni Scheme in .my.* za zasebni Scheme

# -- Help text -- zscheme
help-header-zscheme-topic = ── zscheme ──────────────────────────────────────────────────────────────
help-zscheme-intro = zscheme ovrednoti izraze Scheme, vstavljene v ukaze operator, in pred pošiljanjem vstavi rezultat v vrstico.
