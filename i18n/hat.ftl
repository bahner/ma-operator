...
msg-identity-not-published = Idantite pa jwenn sou entènèt — si ou gen 間 ki instalé lokalman, kouri '.ma: claim [port]' epi '.my.identity!publish @ma'. Tape '.help/publish' pou detay.
...
discover-hint-endpoint-not-found = Sijesyon: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Sijesyon: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Sijesyon: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Sijesyon: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 dekouvri nan { $url }
...
doc-store-sent = demann depo voye ({ $id }) → { $publisher }; CID ap rive via repons
doc-ipld-store-sent = demann depo IPLD voye ({ $id }) → { $publisher }; CID ap rive via repons
...
help-msg-send = @alias!msg body @alias:verb args voye mesaj bay aktè
...
help-config-filter =   .path [non]                  liste non kle timoun ki gen non
help-config-full =   .path. [non]                 liste valè souabre konplè ki gen non
...
help-runtime-discover =   .ma: claim [port]          dekouvri runtime lokal epi kreye alias @ma
help-runtime-claim =   .ma: claim [port]             reklame pwopriyete runtime avèk DID ou
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   konekte ak runtime 間 lokal ou, li /status.json, epi sove .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     pibliye dokiman DID ou pou lòt moun ka rezoud kle ou yo ak endpoint ou
help-ma-security = Limit konfyans ki pi klè a se pwòp runtime 間 ou ak pwòp IPFS Desktop/Kubo ou. Yon publisher aleka ka itil, men lè sa a ou depann de sèvis yon lòt moun.
...
help-topic-ma = .help/ma                     間-espas, pibliye, ak antre
...
help-actor-fragment-verb = @actor#entity:verb [args] bay plugin
