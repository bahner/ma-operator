...
msg-identity-not-published = هویت در شبکه پیدا نشد — اگر 間 را محلی نصب کرده‌اید، '.ma [port]' و سپس '.my.identity!publish @ma' را اجرا کنید. برای جزئیات '.help/publish' را تایپ کنید.
...
profile-no-ma = runtime 間 پیکربندی نشده است — ابتدا '.ma [port]' را اجرا کنید
