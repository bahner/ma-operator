...
new-identity-nick-help = برای ma-space یک نام مستعار انتخاب کنید. این فقط یک لقب است و هر زمان بخواهید می‌توانید آن را تغییر دهید.
...
label-did = DID
label-runtime = ma
label-runtime-placeholder = did:ma:... یا http://localhost:5003
...
msg-auto-published = DID از راه ma محلی منتشر شد ({ $url })
...
msg-ma-checking-url = در حال بررسی ma محلی در { $url }
msg-local-ma-unreachable = ma محلی در { $url } پس از { $seconds } ثانیه در دسترس نبود؛ ma را اجرا کنید یا .ma.ctx.url را تغییر دهید
...
discover-hint-endpoint-not-found = راهنما: endpoint پیدا نشد. بررسی کنید که `ma` اجرا شده و /status.json را روی پورت 5003 ارائه می‌کند.
discover-hint-server-error = راهنما: runtime با خطای سرور پاسخ داد. گزارش‌های `ma` را بررسی کنید و دوباره بکوشید.
discover-hint-network = راهنما: مشکل شبکه یا اتصال. `ma` را اجرا کنید، دسترسی localhost:5003 را بررسی کنید، و دسترسی HTTP محلی را در مرورگر مجاز کنید.
discover-hint-generic = راهنما: بررسی کنید `ma` و IPFS Desktop در حال اجرا هستند، سپس `.ma` را دوباره اجرا کنید.
discover-success = ma در { $url } کشف شد
...
doc-publish-hint-network = بررسی کنید runtime ma و IPFS در دسترس هستند، سپس دوباره بکوشید
...
help-topic-ma =   .help/ma                     ma-space، انتشار، و ورود
...
help-ma-command =   .ma [port]                   به runtime محلی ma وصل شو، /status.json را بخوان، و .ma.ctx.* را ذخیره کن
help-ma-publish =   .my.identity!publish @ma     سند DID خود را منتشر کن تا دیگران بتوانند کلیدها و نقطه پایانی شما را حل کنند
help-ma-security = روشن‌ترین مرز اعتماد، runtime ma خودتان همراه با IPFS Desktop/Kubo خودتان است. ناشر دوردست می‌تواند مفید باشد، اما در آن حالت به سرویس کس دیگری تکیه می‌کنید.
...
help-runtime-discover =   .ma [port]                   اتصال/مطالبه runtime محلی → .ma.ctx.* (پورت پیش‌فرض 5003)
help-runtime-claim =   .ma [port]                   (همان مورد بالا — مطالبه و کشف یکی شده‌اند)
...
profile-no-ma = runtime ma پیکربندی نشده است — ابتدا '.ma [port]' را اجرا کنید
...
help-actor-wc-l =   .my.scheme.ma!edit           ویرایش helperهای ذخیره‌شده Scheme برای این هویت
# ── متن راهنما — انتشار ─────────────────────────────────────────────────
...
help-publish-ma = برای انتشار، باید ma (runtime محلی) نصب باشد. ma از طرف شما ego را به IPFS/IPNS وصل می‌کند.
