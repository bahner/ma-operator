...
msg-local-ma-claimed = lokālais 間 pieprasīts
msg-local-ma-already-claimed = lokālais 間 jau pieprasīts
msg-local-ma-claim-failed = neizdevās pieprasīt lokālo 間
msg-identity-not-published = Identitāte nav atrasta tiešsaistē — ja 間 ir instalēts lokāli, izpildi '.ma: claim [port]' un pēc tam '.my.identity!publish @ma'. Raksti '.help/publish' detaļām.
...
discover-hint-endpoint-not-found = Padoms: endpoint not found. Check that 間 exposes /status.json on port 5003.
discover-hint-server-error = Padoms: runtime returned a server error. Check 間 logs and retry.
discover-hint-network = Padoms: network/connectivity issue. Start 間, verify localhost:5003 is reachable, and allow local HTTP access in the browser.
discover-hint-generic = Padoms: verify 間 and IPFS Desktop are running, then retry `.ma`.
discover-success = 間 atklāts { $url }
...
claim-already-owned = Izpildlaiks jau pieprasīts šai identitātei
...
err-persist = saglabāšanas kļūda: { $e }
...
help-runtime-discover =   .ma: claim [port]          atklāt lokālo izpildlaiku un izveidot @ma aizstājvārdu
help-runtime-claim =   .ma: claim [port]             pieprasīt izpildlaiku ar savu DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   pieslēdzies lokālajam 間 runtime, nolasi /status.json un saglabā .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     publicē savu DID dokumentu, lai citi varētu atrast tavas atslēgas un endpoint
help-ma-security = Skaidrākā uzticēšanās robeža ir tavs paša 間 runtime ar tavu paša IPFS Desktop/Kubo. Attāls publisher var būt noderīgs, bet tad tu paļaujies uz cita cilvēka pakalpojumu.
...
help-actor-text = @actor[#entity]!msg|!say|!emote body         nosūtīt tiešo/tērzēšanas/emociju ziņojumu
...
# ── Batch mode ────────────────────────────────────────────────────────────
batch-collecting-started = Pakešu apkopošana — ierakstiet komandas, beidzas ar .batch
batch-already-collecting = Jau tiek savākta partija — vispirms beidziet ar .batch
batch-empty = Partija bija tukša — nav ko palaist
batch-running = Uruchamianie batch…
batch-step-timeout = partijas darbības noildze
