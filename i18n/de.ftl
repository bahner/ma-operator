...
new-identity-nick-help = Wähle einen Spitznamen für 間-Raum. Es ist nur ein Spitzname und kann jederzeit geändert werden.
...
qr-scan-hint = Richten Sie die Kamera auf einen operator-Profil-QR-Code.
...
msg-logged-in = operator v{ $version } — angemeldet als { $username }
...
msg-auto-published = DID über lokales 間 veröffentlicht ({ $url })
...
msg-trusted-ma-lookup-wait = Die Suche nach dem vertrauenswürdigen MA kann bis zu { $seconds } Sekunden dauern.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Identität wird veröffentlicht. Das kann bis zu { $seconds } Sekunden dauern.
...
msg-identity-not-published = Identität nicht online gefunden — wenn 間 lokal installiert ist, führe '.ma: claim [port]' und dann '.my.identity!publish @ma' aus. Tippe '.help/publish' für Details.
...
discover-hint-endpoint-not-found = Hinweis: Endpunkt nicht gefunden. Prüfe, dass 間 läuft und /status.json auf Port 5003 bereitstellt.
discover-hint-server-error = Hinweis: Die Laufzeit antwortete mit einem Serverfehler. Prüfe die 間-Logs und versuche es erneut.
discover-hint-network = Hinweis: Netzwerk-/Verbindungsproblem. Starte 間, prüfe localhost:5003 und erlaube lokalen HTTP-Zugriff im Browser.
discover-hint-generic = Hinweis: Prüfe, dass 間 und IPFS Desktop laufen, und versuche `.ma` erneut.
discover-success = 間 erkannt bei { $url }
...
doc-publish-hint-session = melde dich erneut an, damit operator auf deine Identitätsschlüssel zugreifen kann
doc-publish-hint-target = verwende eine gültige Publisher-DID oder einen Alias, der zu einer reinen did:ma:<ipns> auflöst
doc-publish-hint-network = prüfe, dass 間-Laufzeit und IPFS erreichbar sind, und versuche es erneut
...
help-header-operator = ── operator-Befehle ───────────────────────────────────────────────────────────
...
help-header-ma = ── 間-Raum ──────────────────────────────────────────────────────────────
help-header-ma-entry = ── Betreten des 間-Raum ────────────────────────── ───────────────────────────
help-footer = ─────────────────────────────────────────────────────────────────────────
...
help-runtime-discover =   .ma: claim [port]          lokale Laufzeit erkennen und @ma-Alias erstellen
help-runtime-claim =   .ma: claim [port]             Laufzeit mit Ihrer DID beanspruchen
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   mit deinem lokalen ma-Runtime verbinden, /status.json lesen und .ma.ctx.* speichern
...
help-ma-security = Die klarste Vertrauensgrenze ist dein eigener 間-Runtime mit deinem eigenen IPFS Desktop/Kubo. Ein entfernter Publisher kann nützlich sein, aber dann verlässt du dich auf den Dienst einer anderen Person.
...
help-topic-url =   .help/url                    operator über einen URL-Link öffnen
...
help-url-intro =   Link teilen, der operator mit vorausgefülltem Empfänger öffnet:
...
help-publish-ma = Zum Veröffentlichen benötigst du 間 (lokale Laufzeit). Es verbindet operator mit IPFS/IPNS in deinem Auftrag.
help-publish-steps = Schritte: führe '.ma: claim [port]' aus, um dein lokales 間 zu finden, dann '.my.identity!publish @ma'.
...
help-zscheme-intro = zscheme wertet Scheme-Ausdrücke aus, die in operator-Befehle eingebettet sind, und fügt das Ergebnis vor dem Senden in die Zeile ein.
