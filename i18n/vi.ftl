...
msg-identity-not-published = Không tìm thấy danh tính trực tuyến — nếu 間 được cài đặt cục bộ, hãy chạy '.ma [port]' rồi '.my.identity!publish @ma'. Gõ '.help/publish' để xem chi tiết.
...
help-ma-command =   .ma [port]                   kết nối với 間 runtime cục bộ, đọc /status.json và lưu .ma.ctx.*
