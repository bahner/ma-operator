...
msg-identity-not-published = Identitas tidak ditemukan secara online — jika 間 terpasang secara lokal, jalankan '.ma [port]' lalu '.my.identity!publish @ma'. Ketik '.help/publish' untuk detail.
...
help-ma-command =   .ma [port]                   hubungkan ke 間 runtime lokal, baca /status.json, dan simpan .ma.ctx.*
...
help-publish-steps = Langkah: jalankan '.ma [port]' untuk mendeteksi 間 lokal, lalu '.my.identity!publish @ma'.
