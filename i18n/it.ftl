# operator — Italiano
lang-name = Italiano
label-nick = soprannome
new-identity-nick-help = Scegli un soprannome per 間-space. È solo un soprannome e puoi cambiarlo quando vuoi.
error-nick-required = soprannome richiesto
error-nick-invalid = il soprannome non deve contenere spazi o @
...
btn-scan-qr = scansiona QR
qr-scan-hint = Punta la fotocamera su un codice QR di un profilo operator.
qr-error-camera = fotocamera non disponibile: { $e }
qr-error-too-large = profilo troppo grande per un codice QR
...
msg-logged-in = operator v{ $version } — connesso come { $username }
...
msg-auto-published = DID pubblicato tramite 間 locale ({ $url })
msg-identity-publication-propagating = La pubblicazione dell’identità è ancora in propagazione; ingresso ritardato: { $e }
msg-trusted-ma-searching = Searching for trusted MA ({ $did })
msg-trusted-ma-lookup-wait = La ricerca del MA fidato può richiedere fino a { $seconds } secondi.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Pubblicazione dell’identità. Può richiedere fino a { $seconds } secondi.
...
msg-identity-not-published = Identità non trovata online — se hai 間 installato localmente, esegui '.ma: claim [port]' poi '.my.identity!publish @ma'. Digita '.help/publish' per i dettagli.
...
discover-hint-endpoint-not-found = Suggerimento: endpoint non trovato. Verifica che 間 sia in esecuzione ed esponga /status.json sulla porta 5003.
discover-hint-server-error = Suggerimento: il runtime ha risposto con un errore del server. Controlla i log di 間 e riprova.
discover-hint-network = Suggerimento: problema di rete/connettività. Avvia 間, verifica che localhost:5003 sia raggiungibile e consenti l’accesso HTTP locale nel browser.
discover-hint-generic = Suggerimento: verifica che 間 e IPFS Desktop siano in esecuzione, poi riprova `.ma`.
discover-success = 間 rilevato a { $url }
...
err-persist = errore di salvataggio: { $e }
...
# ── Verbi — documenti ────────────────────────────────────────────────────
doc-content-empty = { $path } è vuoto
doc-save-first = { $path } è vuoto — salva prima
...
doc-publish-hint-session = accedi di nuovo così operator può accedere alle tue chiavi di identità
doc-publish-hint-target = usa un DID di pubblicatore valido o un alias che risolva a un did:ma:<ipns> senza frammento
doc-publish-hint-network = verifica che runtime 間 e IPFS siano raggiungibili e riprova
...
help-header-operator = ── comandi operator ───────────────────────────────────────────────────────────
...
help-runtime-discover =   .ma: claim [port]          scopri runtime locale e crea alias @ma
help-runtime-claim =   .ma: claim [port]             reclama runtime con il tuo DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   connetti il runtime 間 locale, leggi /status.json e salva .ma.ctx.*
...
help-ma-security = Il confine di fiducia più chiaro è il tuo runtime 間 con il tuo IPFS Desktop/Kubo. Un publisher remoto può essere utile, 間 in quel caso dipendi dal servizio di qualcun altro.
...
help-topic-z =   .help/z                      contenuti Scheme pubblici e privati
help-topic-zscheme =   .help/zscheme               espressioni Scheme inline e documentazione
help-topic-url =   .help/url                    aprire operator tramite un link URL
...
help-url-intro =   Condividi un link che apre operator con il destinatario già compilato:
...
help-publish-ma = Per pubblicare hai bisogno di 間 (runtime locale) installato. Collega operator a IPFS/IPNS per conto tuo.
help-publish-steps = Passi: esegui '.ma: claim [port]' per rilevare il tuo 間 locale, poi '.my.identity!publish @ma'.
...
msg-outbox-open-timeout = l'attore { $target } è irraggiungibile — la connessione è scaduta dopo { $ms }ms
msg-outbox-send-timeout = l'invio a { $target } è scaduto dopo { $ms }ms — la connessione potrebbe essersi interrotta
err-unknown-command = comando sconosciuto: { $path }
err-read-only-path = { $path } è di sola lettura
help-cmd-batch-async =   .batch!async [timeout=60s] / .batch        valuta documento temporaneo in parallelo
...
help-z-public =   .z.*                       contenuti pubblici o pubblicabili; non salvare segreti qui
help-z-scheme =   .z.scheme                  lo script Scheme caricato e valutato all’avvio
help-z-control =   .z.scheme                 sei tu a controllare il contenuto dello script di avvio
help-z-private =   .my.*                     configurazione locale privata; puoi salvare Scheme anche qui
help-z-choice =   .z.* / .my.*               usa .z.* per Scheme pubblico e .my.* per Scheme privato
help-header-zscheme-topic = ── zscheme ──────────────────────────────────────────────────────────────
help-zscheme-intro = zscheme valuta le espressioni Scheme inserite nei comandi operator e innesta il risultato nella riga prima dell’invio.
