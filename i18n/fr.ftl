...
lang-name = Français
label-nick = surnom
new-identity-nick-help = Choisissez un surnom pour 間-space. Ce n'est qu'un surnom et vous pouvez le changer à tout moment.
...
qr-scan-hint = Pointez la caméra vers un code QR de profil operator.
...
msg-logged-in = operator v{ $version } — connecté en tant que { $username }
...
msg-auto-published = DID publié via 間 local ({ $url })
...
msg-trusted-ma-lookup-wait = La recherche du MA de confiance peut prendre jusqu’à { $seconds } secondes.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Publication de l’identité. Cela peut prendre jusqu’à { $seconds } secondes.
...
msg-identity-not-published = Identité introuvable en ligne — si 間 est installé localement, exécute '.ma: claim [port]' puis '.my.identity!publish @ma'. Tape '.help/publish' pour les détails.
...
discover-hint-endpoint-not-found = Indice : point de terminaison introuvable. Vérifiez que 間 fonctionne et expose /status.json sur le port 5003.
discover-hint-server-error = Indice : le runtime a répondu par une erreur serveur. Consultez les journaux de 間 et réessayez.
discover-hint-network = Indice : problème réseau/connexion. Démarrez 間, vérifiez que localhost:5003 est accessible et autorisez l’accès HTTP local dans le navigateur.
discover-hint-generic = Indice : vérifiez que 間 et IPFS Desktop fonctionnent, puis réessayez `.ma`.
discover-success = 間 découvert à { $url }
...
err-persist = erreur d'enregistrement : { $e }
...
doc-publish-hint-session = reconnectez-vous pour qu’operator puisse accéder à vos clés d’identité
doc-publish-hint-target = utilisez une DID de publication valide ou un alias qui se résout en did:ma:<ipns> nu
doc-publish-hint-network = vérifiez que le runtime 間 et IPFS sont accessibles, puis réessayez
...
help-header-operator = ── commandes operator ─────────────────────────────────────────────────────────
...
help-runtime-discover =   .ma: claim [port]          découvrir l'exécution locale et créer l'alias @ma
help-runtime-claim =   .ma: claim [port]             revendiquer l'exécution avec votre DID
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   se connecter au runtime 間 local, lire /status.json et enregistrer .ma.ctx.*
help-ma-publish =   .my.identity!publish @ma     publier ton document DID pour que les autres résolvent tes clés et ton endpoint
help-ma-security = La limite de confiance la plus claire est ton propre runtime 間 avec ton propre IPFS Desktop/Kubo. Un publisher distant peut être utile, mais tu dépends alors du service de quelqu’un d’autre.
...
help-topic-url =   .help/url                    ouvrir operator via un lien URL
...
help-url-intro =   Partage un lien qui ouvre operator avec un destinataire pré-rempli :
...
help-publish-ma = Pour publier, tu as besoin de 間 (runtime local) installé. Il relie operator à IPFS/IPNS en ton nom.
help-publish-steps = Étapes : lance '.ma: claim [port]' pour détecter ton 間 local, puis '.my.identity!publish @ma'.
...
help-zscheme-intro = zscheme évalue les expressions Scheme intégrées aux commandes operator et insère le résultat dans la ligne avant son envoi.
