...
msg-auto-published = DID가 로컬 間 ({ $url })를 통해 게시되었습니다
...
msg-trusted-ma-lookup-wait = 신뢰할 수 있는 MA 조회는 최대 { $seconds }초가 걸릴 수 있습니다.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = identity를 공개하는 중입니다. 최대 { $seconds }초가 걸릴 수 있습니다.
...
msg-identity-not-published = 온라인에서 신원을 찾을 수 없음 — ma가 로컬에 설치되어 있다면, '.ma: claim [port]' 후 '.my.identity!publish @ma'를 실행하세요. 자세한 내용은 '.help/publish'를 입력하세요.
...
discover-hint-endpoint-not-found = 힌트: 엔드포인트를 찾을 수 없습니다. 間가 실행 중이고 5003 포트에서 /status.json을 제공하는지 확인하세요.
discover-hint-server-error = 힌트: 런타임이 서버 오류로 응답했습니다. 間 로그를 확인하고 다시 시도하세요.
discover-hint-network = 힌트: 네트워크/연결 문제입니다. 間를 시작하고 localhost:5003에 접근 가능한지 확인한 뒤 브라우저에서 로컬 HTTP 접근을 허용하세요.
discover-hint-generic = 힌트: 間와 IPFS Desktop이 실행 중인지 확인한 뒤 `.ma`를 다시 시도하세요.
discover-success = { $url }에서 間 발견됨
...
help-runtime-discover =   .ma: claim [port]          로컬 런타임 검색 및 @ma 생성
help-runtime-claim =   .ma: claim [port]             DID로 런타임 소유권 청구
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   로컬 ma runtime에 연결하고 /status.json을 읽은 뒤 .ma.ctx.* 저장
help-ma-publish =   .my.identity!publish @ma     다른 이들이 키와 endpoint를 확인할 수 있도록 DID 문서를 게시
help-ma-security = 가장 분명한 신뢰 경계는 자신의 IPFS Desktop/Kubo와 함께 쓰는 자신의 間 runtime입니다. 원격 publisher가 유용할 수 있지만, 그때는 다른 사람의 서비스에 의존하게 됩니다.
