...
btn-show-qr = 顯示 QR 碼
btn-scan-qr = 掃描 QR 碼
qr-scan-hint = 將相機對準 operator 設定檔 QR 碼。
...
# ── 終端機系統訊息 ────────────────────────────────────────────────────────
msg-logged-in = operator v{ $version } — 已以 { $username } 身分登入
...
msg-identity-not-published = 未在線找到身份 — 如果本地安裝了 間，請執行 '.ma: claim [port]' 然後 '.my.identity!publish @ma'。輸入 '.help/publish' 查看詳情。
...
doc-publish-hint-session = 請重新登入，讓 operator 可以存取你的身分金鑰
...
# ── 說明文字 — 標題 ───────────────────────────────────────────────────────
help-header-operator = ── operator 指令 ─────────────────────────────────────────────────────────────
...
help-runtime-discover =   .ma: claim [port]          探索本地執行階段並建立 @ma 別名
help-runtime-claim =   .ma: claim [port]             用您的 DID 宣告執行階段所有權
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   連接本機 間 runtime，讀取 /status.json，並儲存 .ma.ctx.*
...
help-topic-z =   .help/z                      公開與私人的 Scheme 內容
help-topic-zscheme =   .help/zscheme               內嵌 Scheme 表達式與文件
help-topic-url =   .help/url                    透過 URL 連結開啟 operator
...
help-url-intro =   分享一個連結，開啟 operator 時自動填入收件人：
...
help-publish-ma = 發布需要安裝 間（本地執行環境）。它代表你將 operator 連接到 IPFS/IPNS。
help-publish-steps = 步驟：執行 '.ma: claim [port]' 偵測本地 ma，然後 '.my.identity!publish @ma'。
...
msg-outbox-open-timeout = 無法聯繫到行動方 { $target } — 連線在 { $ms }ms 後逾時
msg-outbox-send-timeout = 發送給 { $target } 在 { $ms }ms 後逾時 — 連線可能已斷開
...
# -- Help text -- z and my
help-header-z = ── .z 與 .my ───────────────────────────────────────────────────────────────────────────
help-z-public =   .z.*                       公開或可發佈的內容；請勿在此儲存秘密
help-z-scheme =   .z.scheme                  啟動時載入並求值的 Scheme 指令碼
help-z-control =   .z.scheme                 您自己控制啟動指令碼的內容
help-z-private =   .my.*                     私人本地配置；Scheme 也可儲存於此
help-z-choice =   .z.* / .my.*               公開 Scheme 使用 .z.*，私人 Scheme 使用 .my.*

# -- Help text -- zscheme
help-header-zscheme-topic = ── zscheme ──────────────────────────────────────────────────────────────
help-zscheme-intro = zscheme 會求值嵌入在 operator 指令中的 Scheme 表達式，並在送出前把結果接回該行。
