...
qr-scan-hint = Pek kameraet mot en operator-profil-QR-kode.
...
msg-logged-in = operator v{ $version } — logget inn som { $username }
...
msg-trusted-ma-lookup-wait = Betrodd MA-oppslag kan ta opptil { $seconds } sekunder.
msg-trusted-ma-not-discovered = Trusted MA was not discovered. The lookup may have failed temporarily. Please try `.ma` again.
msg-identity-first-publish = Publiserer identitet. Dette kan ta opptil { $seconds } sekunder.
...
msg-identity-not-published = Identitet ikke funnet online — om du har 間 installert lokalt, kjør '.ma: claim [port]' og deretter '.my.identity!publish @ma'. Skriv '.help/publish' for detaljer.
...
doc-save-first = { $path } er tom — lagre først
...
doc-publish-hint-session = logg inn på nytt så operator får tilgang til identitetsnøklene dine
...
doc-store-sent = lagringsforespørsel sendt ({ $id }) → { $publisher }; CID ankommer via -svar
doc-ipld-store-sent = IPLD-lagringsforespørsel sendt ({ $id }) → { $publisher }; CID ankommer via -svar
doc-fetch-done = hentet { $cid } → { $path } (ikke kjørt)
...
help-header-operator = ── operator-kommandoer ────────────────────────────────────────────────────────
...
# ── Hjelpetekst — operator-kommandoer ─────────────────────────────────────────
...
help-cmd-history =   .history [n]                 kommandohistorikk (påfølgende duplikater slås sammen); valgfri n viser bare de siste n linjene
...
help-msg-send = @alias!msg body @alias:verb args send melding til aktør
...
help-ma-command =   .ma: claim [port] / .ma: did:ma:… / .ma                   koble til lokal 間-runtime, les /status.json, og lagre .ma.ctx.*
...
help-runtime-discover =   .ma: claim [port]          oppdag lokal kjøretid og opprett @ma-alias
help-runtime-claim =   .ma: claim [port]             gjør krav på kjøretid med din DID
...
help-i18n-set =   .my.i18n: <kode>             velg språket operator bruker for denne identiteten
...
err-history-count-invalid = historikkantallet må være et positivt heltall
...
profile-no-ma = ingen 間-kjøretid konfigurert — kjør '.ma: claim [port]' først
...
help-topic-z =   .help/z                      offentlig og privat Scheme-innhold
help-topic-zscheme =   .help/zscheme               Scheme-uttrykk i kommandoer og dokumentasjon
help-topic-url =   .help/url                    åpne operator via en URL-lenke
...
help-actor-cat = (@actor#entity:verb arg...) kall entity- fra Scheme og vent på svar
...
help-actor-wc = (define x (@actor:verb arg)) behold -svar i session-miljøet
help-header-url = ── URL-parametere ──────────────────────────────────────────────────────────
help-url-intro =   Del en lenke som åpner operator med ferdig utfylt mottaker:
...
help-publish-ma = For å publisere trenger du 間 (lokal kjøretid) installert. Den kobler operator til IPFS/IPNS på dine vegne.
help-publish-steps = Steg: kjør '.ma: claim [port]' for å oppdage din lokale 間, deretter '.my.identity!publish @ma'.
...
msg-outbox-open-timeout = aktøren { $target } er ikke tilgjengelig — tilkoblingen fikk tidsavbrudd etter { $ms }ms
msg-outbox-send-timeout = sending til { $target } fikk tidsavbrudd etter { $ms }ms — tilkoblingen kan ha falt ut
...
help-z-public =   .z.*                       offentlig eller publiserbart innhold; ikke lagre hemmeligheter her
help-z-scheme =   .z.scheme                  Scheme-skriptet som lastes inn og evalueres ved oppstart
help-z-control =   .z.scheme                 du styrer selv innholdet i oppstartsskriptet
help-z-private =   .my.*                     privat lokal konfigurasjon; Scheme kan også lagres her
help-z-choice =   .z.* / .my.*               bruk .z.* for offentlig Scheme og .my.* for privat Scheme
help-header-zscheme-topic = ── zscheme ──────────────────────────────────────────────────────────────
help-zscheme-intro = zscheme evaluerer Scheme-uttrykk som er skrevet inn i operator-kommandoer, og fletter resultatet inn i linjen før den sendes.
