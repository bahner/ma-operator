...
The idea was: when `.use @actor` activates, load the remote actor's zscheme file (fetched via `@actor/zscheme`) and swap it in as the active session environment, reverting to the user's local `.z.scheme` composition on deactivate.
...
`#concourse` is the conventional entry/`start` room fragment (previously `#construct`); it is distinct from `Construct`, the empty-room display name owned by the runtime world profile. Do not confuse or conflate the two.
